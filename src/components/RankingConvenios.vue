    ...
    <div class="container-main">
        ...
        <form class="ranking-form-container">
            <div class="label-form"> 
                ...
                <Dropdown class="dropdown" v-model="startYear" :options="yearOptions" placeholder="Data Inicio..." dateFormat="yy"
                    showIcon inputId="startYear" iconDisplay="input" />
            </div>
            <div class="label-form">
                <label for="endYear"> Até: </label>
                <Dropdown class="dropdown" v-model="endYear" :options="yearOptions" placeholder="Data Fim..." dateFormat="yy" showIcon
                    inputId="endYear" iconDisplay="input" />
            </div>

            <div class="label-form">
                <label for="limit"> Quantidade: </label>
                <Dropdown class="dropdown" v-model="limit" :options="limitOptions" placeholder="Quantidade..."  showIcon
                    inputId="limit" iconDisplay="input" />
            </div>


            <Button @click.lazy="generateRanking" type="button" label="Exibir Ranking" class="btn-ranking"></Button>
        </form>
        ...
        <div class="data-table-container" v-if="isLoaded">
            <DataTable :value="ifesRankingFormated" stripedRows showGridLines size="small" ref="dtIfes" exportFilename="ranking-ifes">
                <template #header>
                    <div class="container-table-header">
                        <h3>Ranking de Universidades</h3>
                        <div style="text-align: left; margin-left: 1em">
                            <Button icon="pi pi-external-link" severity="success" label="Exportar Tabela" id="ranking-ifes" @click="exportarCSV($event)"></Button>
                        </div>
                    </div>
                </template>

                <template #empty> Não foi possível rankear Universidades. Nenhum Historico de Convenios encontrado no período informado </template>
        ...
        <div class="data-table-container" v-if="isLoaded">
            <DataTable :value="convenentesRankingFormated" stripedRows showGridLines size="small" ref="dtConvenentes" exportFilename="ranking-convenentes">
                <template #header>
                    <div class="container-table-header">
                        <h3>Ranking de Convenentes</h3>
                        <div style="text-align: left; margin-left: 1em">
                            <Button icon="pi pi-external-link" severity="success" label="Exportar Tabela" id="ranking-convenentes" @click="exportarCSV($event)"></Button>
                        </div>
                    </div>
                </template>

                <template #empty> Não foi possível rankear Convenentes. Nenhum Historico de Convenios encontrado no período informado. </template>
    ...
    methods: {
        ...
        async generateRanking() {
            try {
                ...
                const ranking = await new ConvenioService().getConveniosRanking(request);
